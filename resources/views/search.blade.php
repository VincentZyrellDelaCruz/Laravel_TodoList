...
@section('content')
    <h1 class="fw-bolder mb-4">Search Results for {{ $search_name }}</h1>
    ...
    <div class="d-flex mb-5">
        <div class="bg-light ps-3 border border-dark rounded-5 d-flex flex-row">
            <form action="{{ route('todo.search') }}" class="d-flex align-items-center gap-2">
                <input type="text" name="search_name" value="{{ $search_name }}" class="form-control bg-transparent border-0" placeholder="Search Todo Item">
                        ...
                        <div class="col-md-2 d-flex gap-3">
                            @if ($task->status === 'In Progress')
                                <form action="{{ route('todo.check', ['id' => $task->id]) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="isFromCurrentTaskPage" value="true">
